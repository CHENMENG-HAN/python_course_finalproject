...
class DeathCountdownApp:
    ...
    def show(self):
        self.time_left_label = tk.Label(main, text="", font=("time news romen", 14, "bold"))
        self.death_section = customtkinter.CTkFrame(
            master=main,
            fg_color="#AFD3E2",
            width=400,
            height=290
        )
        self.birth = customtkinter.CTkLabel(
            master=main,
            text_color="BLACK",
            font=("Sylfaen", 14),
            bg_color="#AFD3E2",
            text='Please input your birthday (YYYY-MM-DD):'
        )
        self.date_entry = customtkinter.CTkEntry(
            master=main,
            width=170,
            placeholder_text="your birthday",
            fg_color="#AFD3E2",
            border_color="#AFD3E2",
            corner_radius=0,
            text_color="Black"
        ...
        self.male_radiobutton = tk.Radiobutton(
            master=main,
            text="Male",
            background="#AFD3E2",
        ...
        self.female_radiobutton = tk.Radiobutton(
            master=main,
            text="Female",
            background="#AFD3E2",
            variable=self.gender_var, 
        ...
        self.message = tk.Label(main)
        self.death_title = customtkinter.CTkLabel(
            master=main,
            text_color="BLACK",
            text="Death Clock",
        ...
        self.death_section.place(x=250, y=50)
        self.death_title.place(x=365, y=100)
        self.birth.place(x=315, y=140)
        self.date_entry.place(x=310, y=165)
        self.gender_label.place(x=315, y=200)
        self.male_radiobutton.place(x=315, y=235)
        self.female_radiobutton.place(x=400, y=235)
        self.calculate_button.place(x=335, y=283)
        self.time_left_label.place(x=300, y=250 )
        global state
        state = 1
    
    def calculate_time_left(self):
        now = datetime.datetime.now()
        life_expectancy = (83.28 * 365 * 2) if self.gender_var.get() == 'Female' else (76.63 * 365 * 2)
        age = now - self.birthdate
        age_in_years = age.total_seconds() / (60 * 60 * 12)
        time_left = life_expectancy - age_in_years
        if time_left < 0:
            self.time_left_label.config(text="You already exceed the average life span!")
        else:
            self.time_left_label.config(text=f"Following average life span,\nYou only have {time_left:.10f}*12 hours can alive")
            self.message.config(text="Don't waste your time, kid", font=("Helvetica", 10, "bold"))
            self.death_title.destroy()
            self.death_section.destroy()
            self.birth.destroy()
            self.date_entry.destroy()
            self.gender_label.destroy()
            self.female_radiobutton.destroy()
            self.male_radiobutton.destroy()
    ...
    def on_calculate_clicked(self):
        try:
            ...
            self.time_left_label.config(text="Please input the date in YYYY-MM-DD form") 
...
obj_death = DeathCountdownApp(main)  
#function start

def get_current_weather(city):
    #get weathehr by api url
    request_url = f'http://api.openweathermap.org/data/2.5/weather?appid=8ff31f7c6bce8543fb4b3d6531fefc4a&q={city}&units=imperial'
...
def refresh_temp_data():
    ...
    for i in L1:
        ...
        if int(city_datas[i].tempMin) > 28:
            hightemp_warn = 1
    for i in range(1, 23):
        tempData_text.tag_add("tag" + str(i * 2), str(i) + ".28", str(i) + ".30")
        tempData_text.tag_config("tag" + str(i * 2), foreground="red")
        tempData_text.tag_add("tag" + str(i * 2 + 1), str(i) + ".58", str(i) + ".60")
...
checkbutton_12 = customtkinter.CTkRadioButton(
    master=main,
    text="12h",
    text_color="Black",
    bg_color="#AFD3E2",
...
checkbutton_24 = customtkinter.CTkRadioButton(
    master=main,
    text="24h",
    text_color="Black",
    bg_color="#AFD3E2",
...
checkbutton_36 = customtkinter.CTkRadioButton(
    master=main,
    text="36h",
    text_color="Black",
    bg_color="#AFD3E2",
